...
void bfs()
{
    ...
    while(front<=rear)
    {
        ...
        for(int i=0;i<8;i++)
        {
            ...
            if(nx<1||ny<1||nx>n||ny>m||city[nx][ny])continue;            if(flag[nx][ny])            {
            if(nf.hide>=c1||visit[nx][ny][nf.hide+1][nf.move])continue;
            visit[nx][ny][nf.hide+1][nf.move]=true;
            q[++rear]=(node){nx,ny,nf.step+1,nf.hide+1,nf.move};                if(nx==ex&&ny==ey&&check(q[rear],ans))ans=q[rear];            }
            else
            {
...
int main()
{
    ...
    bfs();
    if(!ans.x)puts("-1");
    else printf("%d %d %d\n",ans.step,ans.hide,ans.move);    return 0;
}